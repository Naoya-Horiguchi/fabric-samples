...
for orderer in orderer.example.com orderer2.example.com orderer3.example.com ; do
	docker exec -it $orderer wget -q -O - localhost:8443/metrics > /tmp/isleader
	is_leader="$(grep consensus_etcdraft_is_leader /tmp/isleader | grep mychannel | cut -f2 -d' ')"
	echo "orderer: $orderer, isleader: $is_leader"
	if echo "$is_leader" | grep -q 1 ; then
		current_leader=$orderer
		break
	fi
done

echo "################################################################"
echo "####   KILL LEADER NODE ($current_leader) INTENTIONALLY ########"
echo "################################################################"

if [ ! "$current_leader" ] ; then
	echo "Failed to get current leader node"
	exit 1
fi

docker rm -f $current_leader

echo "###### sleep 10 sec to wait next leader election"
